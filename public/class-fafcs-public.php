...
class FAFCS_Public {
	...
	/**
	 * Future Aim Social Comment system comments template file
	 *
	 ...
	 * @return file
	 */
	public function fafcs_template() {
		 ...
		 */
		$post_types = apply_filters( 'fafcs_supported_post_types', array( 'post', 'page' ) );

		if ( ! is_singular( $post_types ) || 'open' != $post->comment_status ) {
			return FAFCS_PLUGIN_DIR . '/public/fafcs-closed-comments.php';
		}

		return FAFCS_PLUGIN_DIR . '/public/fafcs-comments.php';
	}
	...
	public function comments_script() {
		...
		if ( ! empty( $this->options['app_id'] ) ) {
			...
			$script .= "function loadFAFCSComments() {";
	...
	private function scroll_script() {
		...
		$script = "window.onscroll = function () {
                        var rect = document.getElementById('fafcs_comments').getBoundingClientRect();
                        if (rect.top < window.innerHeight) {
                            var fafcs_div = document.getElementById('fafcs_comments');
                            fafcs_div.innerHTML = '" . $this->comments_area_content() . "';
                            loadFAFCSComments();
	...
	private function click_script() {
		...
		$script = "document.getElementById('fafcs_button').onclick = function() {
                    var fafcs_div = document.getElementById('fafcs_comments');
                    fafcs_div.innerHTML = '" . $this->comments_area_content() . "';
                    loadFAFCSComments();
                };
